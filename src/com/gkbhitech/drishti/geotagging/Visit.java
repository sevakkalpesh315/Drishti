...
import com.gkbhitech.drishti.gdo.SelectCustomerForGDOActivity;
import com.gkbhitech.drishti.gdo.SelectCustomerForGDOActivity.FatchCustomerAsynTask;
...
public class Visit extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edtCustomerSearchBox = (EditText) findViewById(R.id.edt_search);
		
		edtCustomerSearchBox.addTextChangedListener(textWatcher);
		
		FatchCustomerAsynTask fatchCustomerAsynTask = new FatchCustomerAsynTask(Visit.this);
	...
	public class FatchCustomerAsynTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				customers = dataBaseAdapter.getCustomerCity(mApp.getPlant());
		...
		protected void onPostExecute(Void result) {
			...
			if (customers != null && customers.size() > 0) {
				...
				if(Constant.log) Log.i(tag, "adapter created");
				
				for (Customer customer : customers) {
					arrayAdapterCustomerName.add(customer.getCity());
				}
				
				if(Constant.log) Log.i(tag, "adapter filled");
				
				lvCustomer.setAdapter(arrayAdapterCustomerName);
				if(Constant.log) Log.i(tag, "adapter set");
